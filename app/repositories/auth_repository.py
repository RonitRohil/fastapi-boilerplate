...
def revoke_user_session(db, session_id: str):
...
def revoke_all_user_sessions(db, user_id: str):
...
def add_user_token(db, user_token: AddUserToken):
...
def get_user_tokens(db, user_id: str):
    return db.query(UserToken).filter(UserToken.user_id == user_id).all()


def revoke_user_token(db, jti: str):
